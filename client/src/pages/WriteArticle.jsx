...
const WriteArticle = () => {
  const articleLength = [
    ...
    { length: 2200, text: "Medium (800-1200 words)" },
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      setLoading(true);
      const prompt = `Write a detailed, well-structured article about ${input}. The article must be ${selectedLength.text}. Make sure to meet the minimum word count.`;
        ...
        <div className="mt-3 flex gap-3 flex-wrap sm:max-w-9/11">
          {articleLength.map((item, index) => (
            <span
              ...
              className={`text-xs px-4 py-1 border rounded-lg cursor-pointer ${selectedLength.text === item.text ? "bg-gray-100 text-slate-700" : "text-gray-500 border-gray-300"}`}
              key={index}
            >
              {item.text}
            </span>
